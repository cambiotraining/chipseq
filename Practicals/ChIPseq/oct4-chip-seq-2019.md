...
The indexed chromosome is generated using the command (you need to uncompressed the fasta file first):
...
```ngs
gunzip Oct4.fastq.gz
bowtie -m 1 -S bowtie_index/mm10 Oct4.fastq > Oct4.sam
```
...
IGV is a stand-alone genome browser. Please check their website [http://www.broadinstitute.org/igv/](http://www.broadinstitute.org/igv/) for all the formats that IGV can display. For our visualisation purposes we will use the BAM and bigWig formats.
...
Open the Excel peak file and view the peak details. Note that the number of tags (column 6) refers to the number of reads at the summit (i.e. summit height).
...
## Questions
1. Does the first peak that was called convincing to you?
\hrulefill\par
...
library(ChIPseeker)
library(TxDb.Dmelanogaster.UCSC.mm10.ensGene)
txdb <- TxDb.Dmelanogaster.UCSC.mm10.ensGene
...
geneSymbols <- data.frame(mapIds(org.Mm.eg.db, gene, 'SYMBOL', 'ENTREZID'))
write.table(geneSymbols[,1], file = "Oct4_peaks_to_genes.txt", sep = "",quote = FALSE, eol = "\n", na = "NA", row.names = FALSE, col.names = FALSE)
```
...
size from mm10.chromInfo"  > mm10.genome
...
To analyse the ChIP-seq profile, the first step is to create a bigwig file from the BAM file that is going
...
METRICS_FILE=Input.sorted.picard.matric  ASSUME_SORTED=true REMOVE_DUPLICATES=true
```
